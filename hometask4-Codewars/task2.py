...
class PokeScan:
    pokeCollection = {'fire': 'fiery', 'water': 'wet', 'grass': 'grassy'}
    def __init__(self, name, level, pkmntype):
        self.name = name
        self.level = level
        self.processed_pkmntype = PokeScan.pokeCollection[pkmntype]

    def info(self):
        pokpower = ""
        if self.level <= 20:
            pokpower = "weak"
        elif 50 >= self.level > 20:
            pokpower = "fair"
        else:
            pokpower = "strong"

        result = "{}, a {} and {} Pokemon.".format(self.name, self.processed_pkmntype, pokpower)
        return result

pok = PokeScan("bulbasaur", 12, "water")
print(pok.info())
